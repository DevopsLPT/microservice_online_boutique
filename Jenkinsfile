pipeline {
    ...
    environment {
        ...
        DOCKER_IMAGE_NAME = "harbor.lptdevops.website/online_boutique/productcatalogservice"
        DOCKER_IMAGE_TAG = "latest"
        REPORT_TRIVY_NAME = "online_boutique_productcatalogservice_trivy_report"
        ...
        stage('Sonarqube_Static_Code_Analysis') {
            steps {
                withCredentials([
                    ...
                    string(credentialsId: 'SONAR_TOKEN_PRODUCTCATALOG', variable: 'SONAR_TOKEN'),
                    string(credentialsId: 'SONAR_PROJECTKEY_PRODUCTCATALOG', variable: 'SONAR_PROJECTKEY')
                ]) {
                    script {
                        sh """
                            docker run --rm \
