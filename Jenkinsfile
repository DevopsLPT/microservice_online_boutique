pipeline {
    ...
    environment {
        PROJECT = "cartservice"
        DOCKER_IMAGE_NAME = "harbor.lptdevops.website/online_boutique/cartservice"
        DOCKER_IMAGE_TAG = "latest"
        REPORT_TRIVY_NAME = "online_boutique_cartservice_trivy_report"
        ...
        stage('Sonarqube_Static_Code_Analysis') {
            steps {
                withCredentials([
                    ...
                    string(credentialsId: 'SONAR_TOKEN_CART', variable: 'SONAR_TOKEN'),
                    string(credentialsId: 'SONAR_PROJECTKEY_CART', variable: 'SONAR_PROJECTKEY')
                ]) {
                    script {
                        sh """
                            docker run --rm \
