pipeline {
    ...
    environment {
        PROJECT = "adservice"
        DOCKER_IMAGE_NAME = "harbor.lptdevops.website/online_boutique/adservice"
        DOCKER_IMAGE_TAG = "latest"
        REPORT_TRIVY_NAME = "online_boutique_adservice_trivy_report"
        ...
        stage('Sonarqube_Static_Code_Analysis') {
            steps {
                withCredentials([
                    ...
                    string(credentialsId: 'SONAR_PROJECTKEY_AD', variable: 'SONAR_PROJECTKEY')
                ]) {
                    script {
                        sh """
                            docker run --rm \
